workspace "Магазин (Ozon)" "Архитектура интернет-магазина, вариант по мотивам ozon.ru" {
    ...
    model {
        ...
        ozonShop = softwareSystem "Магазин" "Интернет-магазин: пользователи, каталог товаров, корзина" {
            ...
            notificationService = container "Notification Service" "Consumer событий UserRegistered и ProductCreated; в учебной реализации логирует полученные события" "Python, AMQP Consumer"
            ...
            messageBroker = container "Message Broker" "Шина событий: UserRegistered, ProductCreated. Гарантия доставки: at-least-once" "RabbitMQ" "MessageBroker"
        }
        ...
        messageBroker -> notificationService "Доставляет UserRegistered и ProductCreated" "AMQP"
        notificationService -> emailService "Может отправлять уведомления пользователям" "HTTPS/SMTP"
    }
    ...
    views {
        ...
        dynamic ozonShop "UserRegistration" "Сценарий: Регистрация пользователя с event-driven уведомлением" {
            ...
            userService -> messageBroker "Publish UserRegistered {userId, login, firstName, lastName}"
            messageBroker -> notificationService "Consume UserRegistered"
            notificationService -> emailService "Отправить приветственное письмо"
